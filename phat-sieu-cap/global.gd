...
func save_game():
	# Use encrypted save with a hardcoded password
	var file = FileAccess.open_encrypted_with_pass(SAVE_PATH, FileAccess.WRITE, "security_password_phat_game")
	if file:
		var data = {
			"coins": coins,
...
func load_game():
	...
	# 1. Try to open as an Encrypted file first
	var file = FileAccess.open_encrypted_with_pass(SAVE_PATH, FileAccess.READ, "security_password_phat_game")

	# 2. If existing file is NOT encrypted (older save), this usually returns status OK but gibberish, or fails.
	# Godot's behavior on reading plain text as encrypted can vary, but usually FileAccess returns null OR get_as_text returns garbage.
	# Let's try to verify via JSON parsing.
